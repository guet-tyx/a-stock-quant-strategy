...
arxiv_deep_factors = pd.read_csv(os.path.expanduser("~/quant_strategies/individual_stock_strategy/data/arxiv_deep_factors.csv"))
arxiv_deep_factors["trade_date"] = pd.to_datetime(arxiv_deep_factors["trade_date"])
...
df = df.merge(arxiv_deep_factors[["ts_code", "trade_date", "short_ma_diff_rank", "gap_factor"]], on=["ts_code", "trade_date"], how="left")
...
# ============================================================
# 策略1: 四因子等权组合 (ML + LLM + arXiv + arXiv_deep)
# ============================================================
print("\n" + "=" * 60)
print("策略1: 四因子等权组合")
print("=" * 60)
...
monthly["arxiv_deep_rank"] = monthly.groupby("trade_date")["short_ma_diff_rank"].rank(pct=True, ascending=False)

# 四因子等权
monthly["quad_combined"] = (
    0.4 * monthly["ml_rank"] + 
    0.2 * monthly["llm_rank"] + 
    0.2 * monthly["arxiv_rank"] + 
    0.2 * monthly["arxiv_deep_rank"]
)

quad_result = backtest_factor(monthly, "quad_combined", top_n=50, ascending=False)
print(f"\n四因子等权组合 (ML40% + LLM20% + arXiv20% + arXiv_deep20%):")
for k, v in quad_result.items():
    print(f"  {k}: {v}")
...
# 根据IC_IR调整权重
monthly["opt_combined"] = (
    0.45 * monthly["ml_rank"] + 
    0.25 * monthly["llm_rank"] + 
    0.2 * monthly["arxiv_rank"] + 
    0.1 * monthly["arxiv_deep_rank"]
)

opt_result = backtest_factor(monthly, "opt_combined", top_n=50, ascending=False)
print(f"\n优化权重组合 (ML45% + LLM25% + arXiv20% + arXiv_deep10%):")
...
other_feature_cols = [
    "liquidity_sentiment", "liquidity_adj_momentum", 
    "short_ma_diff_rank", "gap_factor"
]

all_feature_names = ml_feature_cols + other_feature_cols
...
for _, row in importances.head(10).iterrows():
    ...
    print(f"  {row['feature']:<25} {pct:>6.1f}%")
...
print("所有策略对比")
print("=" * 75)
print(f"{'策略':<35} {'年化收益':>8} {'最大回撤':>8} {'夏普':>6} {'Calmar':>7} {'胜率':>6}")
print("-" * 75)
...
try:
    with open(os.path.expanduser("~/quant_strategies/individual_stock_strategy/data/arxiv_backtest_results.json")) as f:
        arxiv_data = json.load(f)
    triple_eq = arxiv_data.get("triple_combined", {})
except:
    triple_eq = {"annual_return": 46.67, "max_drawdown": -16.64, "sharpe": 1.68, "calmar": 2.80, "win_rate": 69.0}

print(f"{'ML因子策略(纯)':<35} {ml_base['annual_return']:>7.2f}% {ml_base['max_drawdown']:>7.2f}% {ml_base['sharpe']:>6.2f} {ml_base['calmar']:>7.2f} {ml_base['win_rate']:>5.1f}%")
print(f"{'ML+LLM等权组合':<35} {ml_llm_eq['annual_return']:>7.2f}% {ml_llm_eq['max_drawdown']:>7.2f}% {ml_llm_eq['sharpe']:>6.2f} {ml_llm_eq['calmar']:>7.2f} {ml_llm_eq['win_rate']:>5.1f}%")
print(f"{'ML+LLM+arXiv等权组合':<35} {triple_eq['annual_return']:>7.2f}% {triple_eq['max_drawdown']:>7.2f}% {triple_eq['sharpe']:>6.2f} {triple_eq['calmar']:>7.2f} {triple_eq['win_rate']:>5.1f}%")
print(f"{'四因子等权组合':<35} {quad_result['annual_return']:>7.2f}% {quad_result['max_drawdown']:>7.2f}% {quad_result['sharpe']:>6.2f} {quad_result['calmar']:>7.2f} {quad_result['win_rate']:>5.1f}%")
print(f"{'优化权重组合':<35} {opt_result['annual_return']:>7.2f}% {opt_result['max_drawdown']:>7.2f}% {opt_result['sharpe']:>6.2f} {opt_result['calmar']:>7.2f} {opt_result['win_rate']:>5.1f}%")
print(f"{'全因子联合训练':<35} {full_model_result['annual_return']:>7.2f}% {full_model_result['max_drawdown']:>7.2f}% {full_model_result['sharpe']:>6.2f} {full_model_result['calmar']:>7.2f} {full_model_result['win_rate']:>5.1f}%")

# 找出最优策略
all_strategies = {
    "ML因子策略": ml_base,
    "ML+LLM等权": ml_llm_eq,
    "ML+LLM+arXiv等权": triple_eq,
    "四因子等权": quad_result,
    "优化权重": opt_result,
    "全因子联合训练": full_model_result
}

best_sharpe = max(all_strategies.items(), key=lambda x: x[1].get("sharpe", 0))
best_return = max(all_strategies.items(), key=lambda x: x[1].get("annual_return", 0))
best_calmar = max(all_strategies.items(), key=lambda x: x[1].get("calmar", 0))

print("\n" + "=" * 75)
print("🏆 最优策略推荐")
print("=" * 75)
print(f"最高夏普: {best_sharpe[0]} (夏普={best_sharpe[1]['sharpe']:.2f})")
print(f"最高收益: {best_return[0]} (年化={best_return[1]['annual_return']:.2f}%)")
print(f"最高Calmar: {best_calmar[0]} (Calmar={best_calmar[1]['calmar']:.2f})")

# 保存最终结果
final_results = {
    "quad_combined": quad_result,
    "opt_combined": opt_result,
    "full_model": full_model_result,
    "best_strategy": {
        "name": best_sharpe[0],
        "metrics": best_sharpe[1]
    }
}
with open(os.path.expanduser("~/quant_strategies/individual_stock_strategy/data/final_backtest_results.json"), "w") as f:
